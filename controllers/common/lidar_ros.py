...
class LidarRos(object):
    def __init__(self, lidar, frame_id="base_laser_link", zero_value=0.00001, precision=2):
        self.lidar = lidar
    ...
    def get_ros_msg(self):
        ...
        for s in scan_org:
            if s == float('inf') or s == 0:
                scan.append(self.zero_value)
            else:
                scan.append(round(s, self.precision))
        msg = {
            ...
            'intensities': [self.zero_value] * len(scan),
        }
        self.seq = self.seq + 1
        return msg
